...
module avr_interface_1 (
  ...
  wire [1-1:0] M_spi_peripheral_sdo;
  wire [1-1:0] M_spi_peripheral_done;
  wire [8-1:0] M_spi_peripheral_data_out;
  reg [1-1:0] M_spi_peripheral_cs;
  reg [1-1:0] M_spi_peripheral_sdi;
  reg [1-1:0] M_spi_peripheral_sck;
  reg [8-1:0] M_spi_peripheral_data_in;
  spi_peripheral_4 spi_peripheral (
    .clk(clk),
    .rst(n_rdy),
    .cs(M_spi_peripheral_cs),
    .sdi(M_spi_peripheral_sdi),
    .sck(M_spi_peripheral_sck),
    .data_in(M_spi_peripheral_data_in),
    .sdo(M_spi_peripheral_sdo),
    .done(M_spi_peripheral_done),
    .data_out(M_spi_peripheral_data_out)
  );
  ...
  always @* begin
    ...
    M_spi_peripheral_sck = spi_sck;
    M_spi_peripheral_sdi = spi_mosi;
    M_spi_peripheral_data_in = 8'hff;
    M_spi_peripheral_cs = spi_ss;
    M_uart_rx_rx = rx;
    ...
    spi_miso = M_cclk_detector_ready && !spi_ss ? M_spi_peripheral_sdo : 1'bz;
    ...
    if (M_spi_peripheral_done) begin
      if (M_byteCt_q == 1'h0) begin
        M_sampleReg_d[0+7-:8] = M_spi_peripheral_data_out;
        M_byteCt_d = 1'h1;
      end else begin
        if (M_byteCt_q == 1'h1) begin
          M_sampleReg_d[8+1-:2] = M_spi_peripheral_data_out[0+1-:2];
          M_sampleChannelReg_d = M_spi_peripheral_data_out[4+3-:4];
          M_byteCt_d = 2'h2;
